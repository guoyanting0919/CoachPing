...
手機 LIFF 上單一畫面完成，由上而下：選學員 → 選日期 → 選時間與時長 → 選重複的星期幾與週數。不做多步精靈——精靈每一步都要等頁面切換。底部固定列即時顯示「首堂時間 + 共幾堂」預覽與送出鈕。

學員選擇用**可搜尋的多選清單**（全螢幕覆蓋 + 搜尋框），不用橫排標籤：教練可能有數十位學員，標籤會爆版且找不到人。上限 3 位。

重複課程**展開成實體**：設定當下即依教練指定的週數生成 `sessions` 記錄，以 `series_id` 串聯。每筆獨立可改。

持續週數可選 **2～12 週，預設 2 週**。教練明確指定要排幾週，系列即在那時結束、**不自動延長**——因此不需要「每週往後補一週」的 cron。私教的排課視野通常很短，長期學員到期再重排即可。
