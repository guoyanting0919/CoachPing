...
**時長鎖定** `coaches.default_duration`，學員不能選。**一次只能約一堂**，不提供比照排課的連續多週——
連續 12 週等於一次佔走教練 12 個時段，是重大承諾，不該由學員單方面做；且中間任一週撞到封鎖時段
就得定義「部分成功」，那是一整套額外的錯誤語義。
...
堂數包／剩餘堂數扣抵、金流、補課、多層權限、健身房管理端、Google OAuth 雙向同步、團體課／小班、多時區、學員資料編輯、教練自助註冊、**訂閱制與營收報表**、**預約核准制**、**學員連續多週預約**、**預約加開時段（封鎖只能收回）**、**學員加入他人已有的課**。
