...
- **每日彙總不另設 cron**：由每分鐘的派送在台北時間 08 點時順便排入，以「今天是否已排過」保證冪等。多一支排程就多一個會默默失效的東西，而且還要在外部服務裡再設一次時區。只為今天有課的教練排——沒課的人不需要收到「你今天沒有課」。
...
- **今日課表走 postback，不開 LIFF**：由 webhook 直接回一則文字訊息列出今天每一堂的
  起訖時間與學員。教練在健身房現場最常做的就是瞄一眼今天有誰，不該等網頁載入。
  走 reply message（免費），不佔推播額度。教練直接輸入「今日課表」也有同樣效果
